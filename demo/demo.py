...
def get_parser():
    ...
    parser.add_argument("input", metavar="pdf", nargs=1, help="The input PDF the model should be called with.")
    parser.add_argument("output", metavar="out_path", nargs=1, help="The path to which the model's output should be saved. The model will generate a JPEG and a JSON file.")
    parser.add_argument("--use_cuda", default=False, metavar="CUDA", help="Whether or not to use CUDA (Boolean). Defaults to False. If set to False, the model will be called using the CPU. Setting this option to True requires the device to support CUDA.")
